#include<bits/stdc++.h>
using namespace std;
//https://youtu.be/XqkneOGIJG4?si=4iwevy5lj_rU-WPy prefer this video for understanding
...
int main() {
    ...
    for (long long i = 0; i < q; i++) {
        ...
        if (e == 1) {
            ...
            updatess(ee, -1*b, 0, 0, arr.size() - 1);
        } else {
            // Print sum
            long long c;
            cin >> c;
            long long ss = 0;//kyuki hmehsa root se hi to dekhna hai na ji
            long long ee = start[c];
            // Get the sum for this range
            cout << summ(ss, ee, 0, 0, arr.size() - 1)  << " ";   //pichle jisa yha divided by 2 mat krna yha pe..kyuki hmne nullify kr diya hai reapested node ke value ko -value se replace krke
            //if not understood see in above link diya hua hai
            continue;
        }
    }
